...
function sg_get_post_terms($post_id=null, $taxonomy_slug='post_tag', $count=null, $format=''){		
	...
	for($i=0; $i<$count; $i++){
		$output .= sprintf($format, get_term_link($post_cats[$i]->term_id, $taxonomy_slug), $post_cats[$i]->name, $post_cats[$i]->slug);
	}
...
function sg_get_post_tags($post_id=null, $count=null, $format=''){		
	...
	$output = '';
	if(!$format){
		$format = '<a class="tag" href="%s">%s</a>';
	}


	for($i=0; $i<$count; $i++){
		// $output .= '<a class="tag" href="'.get_tag_link($post_cats[$i]->term_id).'">'.$post_cats[$i]->name.'</a>';
